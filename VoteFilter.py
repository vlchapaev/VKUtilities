...
vApi = "5.92"
try:
    result = requests.get('https://api.vk.com/method/polls.getById?poll_id=' + idPoll + "&owner_id=-" + idGroup + "&access_token=" + token + "&v=" + vApi).json()
    time.sleep(0.3)
...
for i in responseGetById["answers"]:
    ...
    while 1:
        try:
            result = requests.get('https://api.vk.com/method/polls.getVoters?count=500&poll_id=' + idPoll + "&offset=" + str(offsetUser) + "&answer_ids=" + str(id) + "&owner_id=" + idGroup + "&access_token=" + token + "&v=" + vApi).json()
            time.sleep(0.3)
        except requests.exceptions.ReadTimeout:
        ...
        userList = responseGetVoters[0].get("users").get("items")
        if len(userList) == 1: 
            break
        offsetUser += 500
        if len(userList) == 0:
        ...
        try:
            result = requests.get('https://api.vk.com/method/groups.isMember?extended=1&group_id=' + idGroup + "&user_ids=" + userIds + "&access_token=" + token + "&v=" + vApi).json()
            time.sleep(0.3)
        except requests.exceptions.ReadTimeout:
